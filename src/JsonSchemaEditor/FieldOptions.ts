 ...
 * @returns
 */
export const toOfName = (schemaMap: Map<string, boolean | JSONSchema6>) => {
 ...
 * @param rootSchema
 */
export const schemaShortable = (ref: string, rootSchema: JSONSchema6 | boolean | undefined) => {
 ...
 * @param schemaCache
 */
export const canSchemaCreate = (props: FieldProps, schemaCache: SchemaCache) => {
  const { data } = props
 ...
 */
export const canSchemaRename = (props: FieldProps, schemaCache: SchemaCache) => {
  const { field, fatherInfo } = props
  const { entrySchemaMap, propertyCache } = schemaCache
  ...
  if (fatherType === 'array') {
    ...
    const { props: properties, patternProps } = propertyCacheValue
 ...
 * @param schemaCache
 */
export const canDelete = (props: FieldProps, schemaCache: SchemaCache) => {
 ...
 * @returns
 */
export const shallowValidate = (data: any, valueEntry: string, schemaCache: InfoContent, deep = true): boolean => {
    ...
    switch (type) {
      case 'object':
        if (deep) {
          ...
          if (propertyCacheValue) {
            const { props, required } = propertyCacheValue
