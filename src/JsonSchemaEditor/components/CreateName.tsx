...
import { addRef, concatAccess, findKeyRefs, getValueByPattern } from '../utils'
...
const CreateName = (props: CreateNameProps) => {
  ...
  const propertyCacheValue = fatherInfo.type === 'object' && valueEntry ? propertyCache.get(valueEntry) : undefined

  let autoCompleteOptions: any[] = []
  if (propertyCacheValue) {
    const { props: properties } = propertyCacheValue
    const optionStrings = props
      ...
      if (itemCacheValue) {
        ...
        const schemaRef =
          itemLength !== undefined
            ? nowLength < itemLength
              ? addRef(valueEntry, 'items', nowLength.toString())
              : addRef(valueEntry, 'additionalItems')
            : addRef(valueEntry, 'items')
  ...
  const handleCreate = () => {
    ...
    if (data[name] !== undefined) {
      ...
      if (propertyCacheValue) {
        ...
        const info = properties[name] ?? getValueByPattern(patternProps, name) ?? additional
        if (!info) {
