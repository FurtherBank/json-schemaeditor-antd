...
const cacheInput = (InputComponent: React.ComponentType<InputComProps>): React.FC<CachedComProps> => {
  return ({ value, onValueChange, validate, onBlur, ...props }) => {
    ...
    const newOnBlur = (e: { currentTarget: { value: any; }; }) => {
      if (validate === true || validate(cache) ) {
        setPrev(cache)
    ...
    return <InputComponent {...props} onChange={onChange} value={cache} onBlur={newOnBlur} />
  }
}
